...
bool GoDown::Step(World* w) {
    ...
    w->SetNodeColor(current, Color::HotPink);
    ...
    if (!neighbors.empty()) {
        ...
        Point2D down{current.x, current.y + 1};
...
Point2D GoDown::randomStartPoint(World* world) {
  auto sideOver2 = world->GetSize() / 2;
...
std::vector<Point2D> GoDown::getVisitables(World* w, const Point2D& p) {
  auto sideOver2 = w->GetSize() / 2;
